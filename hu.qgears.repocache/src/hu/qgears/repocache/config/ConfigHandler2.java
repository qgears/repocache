	...
	protected void initPage() {
		{
			...
			submit.clicked.addListener(new UtilEventListener<QButton>() {
				...
				public void eventHappened(QButton msg) {
					try {
						getHttpQuery().rc.getConfiguration().saveName(editor.text.getProperty());
					} catch (IOException e) {
		...
		}
		{
			...
			buttonAccess.clicked.addListener(new UtilEventListener<QButton>() {
				...
				public void eventHappened(QButton msg) {
					try {
						getHttpQuery().rc.getConfiguration().saveAccessRules(accessRules.text.getProperty());
					} catch (IOException e) {
		...
		
		{
			...
			editorClientAlias.text.setPropertyFromServer(getHttpQuery().rc.getConfiguration().getClientAlias());
			...
			clientAliasButton.clicked.addListener(new UtilEventListener<QButton>() {
				...
				public void eventHappened(QButton msg) {
					try {
						getHttpQuery().rc.getConfiguration().saveClientAlias(editorClientAlias.text.getProperty());
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
		...
		
		{
			...
			editor.text.setPropertyFromServer(getHttpQuery().rc.getConfiguration().getPluginsConfig());
			QButton submit=new QButton(page, "submit-plugins");
			submit.clicked.addListener(new UtilEventListener<QButton>() {
				@Override
				public void eventHappened(QButton msg) {
					try {
						getHttpQuery().rc.getConfiguration().savePluginsConfig(editor.text.getProperty());
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
		...
		}
		{
			...
			editorInternetAlias.text.setPropertyFromServer(getHttpQuery().rc.getConfiguration().getInternetAliases());
